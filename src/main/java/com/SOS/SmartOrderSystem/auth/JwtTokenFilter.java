...
public class JwtTokenFilter extends OncePerRequestFilter { //OncePerRequestFilter 을 상속받아
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        try{
            ...
            // loginUser 정보로 UsernamePasswordAuthenticationToken 발급 , SecurityContext에 추가할 객체
            AbstractAuthenticationToken authenticationToken =
                    new UsernamePasswordAuthenticationToken(loginId, null, AuthorityUtils.NO_AUTHORITIES);
            authenticationToken.setDetails(new WebAuthenticationDetailsSource().buildDetails(request));

            // SecurityContext에 UsernamePasswordAuthenticationToken를 객체를 추가하여
            // 해당 Thread가 지속적으로 인증 정보를 가질 수 있도록 권한 부여
